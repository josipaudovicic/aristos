...
temp <- data.frame(broj_stana = rep(stanovi$broj_stana, 5),
                   ...
                   ciscenje = seq(Sys.Date(), by = "3 weeks", length.out = nrow(stanovi) * 5))
...
set.seed(10)
random_time_zones <- sample(OlsonNames(), 10)
random_time_zones
```
...
before_1_hour <- now() -  3600
rez <- paste(hour(before_1_hour), minute(before_1_hour), round(second(before_1_hour), 0), sep = ":")
cat(rez)
```
...
interval3 <- interval(start = as.POSIXlt("1.5.2002.", format = "%d.%m.%Y."), 
                      end = as.POSIXlt("1.7.2002.", format = "%d.%m.%Y."))
...
```{r}
crime_sample <- read_csv('crimeSample.csv')
#glimpse(crime_sample)
#head(crime_sample)
#str(crime_sample)
crime_sample$Dispatch_Date_Time <- as.POSIXct(crime_sample$Dispatch_Date_Time, format = "%Y-%m-%d %H:%M:%S")
```
...
```{r}
crime_sample$Year <- year(crime_sample$Dispatch_Date_Time)
crime_sample$Month <- month(crime_sample$Dispatch_Date_Time)
crime_sample$Hour <- hour(crime_sample$Dispatch_Date_Time)
head(crime_sample)

table(crime_sample$Month) %>% as.data.frame() %>% `colnames<-`(c("Month", "Frequency")) %>% filter(Frequency == max(Frequency)) %>% pull(Month) %>% as.numeric() %>% print()

table(crime_sample$Hour) %>% as.data.frame() %>% `colnames<-`(c("Hour", "Frequency")) %>% filter(Frequency == max(Frequency)) %>% pull(Hour) %>% as.numeric() %>% print()
```
